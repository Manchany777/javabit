...
public class MemberServiceSam {
	Scanner scan = new Scanner(System.in);
	MemberDTOSam[] ar = new MemberDTOSam[3];
	
	public MemberServiceSam() {
	...
	public void insert() {
		...
		for(i=0; i < ar.length; i++) {
			...
			ar[i] = new MemberDTOSam(name, age, phone, address);
	...
	public void update() {
		...
		for (int i = 0; i < ar.length; i++) {
		   if (ar[i].getPhone().equals(phoneNum)) {  // equals 써서 수정
			   ...
			   //ar[i] = new MemberDTOSam(name, age, phone, address);  // new하면 새로 갱신된 값은 새로 방이 생기고 기존의 값은 연결이 끊어지는 것뿐이다.
			   // 보통 이 끊어진 방은 GC가 제거를 하지만 이 숫자가 수천수만이 되면 GC도 감당이 안되 메모리누수가 일어날 수 밖에 없다.
			   // 이럴경우는 new보다 setter를 쓰는 게 더 좋다.
	...
	public void delete() {
		...
		for (i = 0; i < ar.length; i++) {
			if (ar[i] != null) {
				if(ar[i].getPhone().equals(phoneNum)) {
					ar[i] = null;
